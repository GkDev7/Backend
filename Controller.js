...
const dbUrl = "mongodb+srv://majorProject:TbGWLHNxJzRUhoo9@cluster0-ccc3k.mongodb.net/"
...
updateCourse = (req, res) => {
    const courseRec = req.body
    // console.log(courseRec,"details")
    a1 = { _id: mongodb.ObjectID(courseRec._id) }
    // console.log(a1,"this is id from mongodb")
    a2 = { course: courseRec.course }
    dbConn.collection('course').updateOne(a1, { $set: a2 }, (err, rec) => {
        //  console.log(rec.result)
        if (rec.result.nModified > 0) {
            res.json({ data: rec.ops, msg: "successfully updated", success: true })
        } else {
...
deleteCategory = (req, res) => {
    const catRec = req.body
    dbConn.collection('category').deleteOne({ _id: mongodb.ObjectID(catRec._id) }, (err, rec) => {
        if (rec.deletedCount > 0) {
...
deleteBranch = (req, res) => {
    const branchRec = req.body
    dbConn.collection('branch').deleteOne({ _id: mongodb.ObjectID(branchRec._id) }, (err, rec) => {
        if (rec.deletedCount > 0) {
            res.json({ msg: 'successfully deleted', data: rec.ops, success: true })
        } else {
            res.json({ msg: "something went wrong", data: null, success: false })
            ...
            bcrypt.hash(adminRec.password, 5).then(newPass => {
                ...
                dbConn.collection('admin').insertOne(adminRec, (err, rec) => {
                    if (rec.insertedCount > 0) {
                        ...
                        res.json({ data: rec.ops, msg: "successfully Registerd", success: true, token: token })
                    } else {
                        res.json({ data: null, msg: "something went wrong", success: false })
                    }
                })
    ...
    if (adminRec.email.length > 0 && adminRec.password.length > 0) {
        dbConn.collection('admin').findOne({ email: adminRec.email }, (err, rec) => {
            bcrypt.compare(adminRec.password, rec.password, (err, sts) => {
                // console.log(sts)
                if (sts) {
                    var token = jwt.sign({ email: adminRec.email }, "this_is_our_token")
        ...
        if (rec) {
            ...
            bcrypt.hash(stdRec.password, 5).then(newPass => {
                ...
                dbConn.collection('stdreg').insertOne(stdRec, (err, rec) => {
                    if (rec.insertedCount > 0) {
                        var token = jwt.sign({ email: stdRec.email }, "this_is_our_token")
                        res.json({ data: rec.ops, msg: "successfully Registerd", success: true, token: token })
                    } else {
                        res.json({ data: null, msg: "something went wrong", success: false })
                    }
...
stdLogin = (req, res, next) => {
    const stdRec = req.body
    console.log(stdRec, "yes")
    if (stdRec.email.length > 0 && stdRec.password.length > 0) {
        dbConn.collection('stdreg').findOne({ email: stdRec.email }, (err, rec) => {
            bcrypt.compare(stdRec.password, rec.password, (err, sts) => {
               console.log(sts,"no")
                if (sts) {
                    var token = jwt.sign({ email: stdRec.email }, "this_is_our_token")
                    rec.password = "*****"
...
updateStd = (req, res) => {
    ...
    a2 = { name: stdRec.name, branch: stdRec.branch, college: stdRec.college, education: stdRec.education, }
    console.log(stdRec)
    dbConn.collection("stdreg").updateOne(a1, { $set: a2 }, (err, rec) => {
        if (rec.result.nModified > 0) {
            res.json({ message: "succesfully updated", success: true, data: [stdRec] })
        } else {
...
deleteStdRec = (req, res) => {

    const stdRec = req.body


    dbConn.collection("stdreg").deleteOne({ _id: mongodb.ObjectID(stdRec._id) }, (err, rec) => {
        console.log(rec, "DELETE DETAILS")
        if (rec.deletedCount > 0) {
            res.json({ data: rec.ops, msg: "successfully deleted", success: true })
        } else {
            res.json({ data: [], msg: "something went wrong", success: false })
        }
    })
}
